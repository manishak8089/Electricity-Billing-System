...
$counter = 1;
while ($row = mysqli_fetch_assoc($result)) {
    // Calculate dues if due date is overdue
    $dues = $row['ddate'] < date('Y-m-d') ? ($row['amount'] * 0.005) : 0;

    // Calculate total payable amount
    $totalPayable = $row['amount'] + $dues;
?>
<tr>
    <form action="transact_bill.php" method="post">
        ...
        <input type="hidden" name="dues" value="<?php echo $dues; ?>">
        <td><?php echo $dues; ?></td>

        <!-- Display total payable amount -->
        <input type="hidden" name="payable" value="<?php echo $totalPayable; ?>">
